...
import type { TokenAnalysis } from './types/analysis.js';
import type { ThesisResult } from './types/anthropic.js';
import type { AlertWallet, AlertTransaction } from './types/alerts.js';
...
export class TradeThesisAgent {
  ...
  /**
   * Initialize trade thesis agent
   * @param apiKey - Anthropic API key (defaults to process.env.ANTHROPIC_API_KEY)
   */
   ...
   * @param walletInfo - Wallet that triggered the alert
  ...
  public async generateThesis(
    ...
    try {
      ...
      const thesis = response.content[0]?.type === 'text' ? response.content[0].text : '';
      ...
      return {
        thesis,
        recommendation,
        score: tokenAnalysis.scoring?.score ?? 50,
        aiGenerated: true,
      };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error('Error generating AI thesis:', errorMessage);
      return this.generateFallbackThesis(tokenAnalysis);
    }
  }
  ...
  private buildPrompt(
    ...
    wallet: AlertWallet,
    _transaction: AlertTransaction
  ): string {
    const { token, marketData, volumeAnalysis, whaleActivity, tokenAge, priceAction } =
      analysis;
...
## Price Action
- **5min:** ${priceAction && priceAction.change5m > 0 ? '+' : ''}${priceAction?.change5m.toFixed(1)}%
- **1H:** ${priceAction && priceAction.change1h > 0 ? '+' : ''}${priceAction?.change1h.toFixed(1)}%
- **24H:** ${priceAction && priceAction.change24h > 0 ? '+' : ''}${priceAction?.change24h.toFixed(1)}%

## Whale Activity
${whaleActivity && whaleActivity.whaleCount > 0 ? `**${whaleActivity.whaleCount} other tracked whale(s) holding this token:**
...
Keep it concise and actionable. Focus on the market cap range, volume momentum, and whale activity.`;
   ...
   */
  private extractRecommendation(thesis: string): 'BUY' | 'WATCH' | 'SKIP' {
    if (thesis.includes('RECOMMENDATION:** BUY') || thesis.includes('RECOMMENDATION: BUY')) {
      return 'BUY';
    } else if (
      thesis.includes('RECOMMENDATION:** WATCH') ||
      thesis.includes('RECOMMENDATION: WATCH')
    ) {
      return 'WATCH';
    } else {
   ...
   */
  private generateFallbackThesis(analysis: TokenAnalysis): ThesisResult {
...
_AI thesis generation disabled (no API key)_`;
   ...
   */
  public formatForTelegram(thesisResult: ThesisResult): string {
    ...
    const emoji =
      thesisResult.recommendation === 'BUY'
        ? '🟢'
